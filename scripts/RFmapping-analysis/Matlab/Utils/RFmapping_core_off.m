function RFmapping_core_off(params)
    ...
    for session_raw = sessionList
        ...
        for probe = probelist
            ...
            save_dir = [base_dir, 'data/rfmapping_off/', unitSelectionFolder, '/', timeFolder, '/Probe', probe, '/'];
        ...
        %%%%%egocentric%%%%%
            ...
            parfor k = 1:unitNum
                ...
                unitRF = [];
                % unitRF.OFF.OnSet = zeros(30, 7, 25);
                unitRF.OFF.OnSet = zeros(y_num,x_num,nbins);
        ...
        %%%%%egocentric%%%%%
                            ...
                            IDoff = inSquareX & VisStim.PosY == curY & VisStim.Lum == 0;
        %%%%%360bin%%%%%
                        elseif isAllocentricPixelBins
                            curX = xPositions(x);
                            curY = yPositions(y);
                        ...
                        % IDoff = VisStim.PosX == curX & VisStim.PosY == curY & VisStim.Lum == 0;
                        [sync, i] = Sync(s, VisStim.periods(IDoff, 1), 'durations', VSTimeWindow);
                        [hist, ~] = SyncHist(sync, i, 'durations', VSTimeWindow, 'nBins', nbins);
        
                        if ~isempty(hist)
                            unitRF.OFF.OnSet(y, x, :) = hist;
                        end
            ...
            if isPixelByPixelAnalysis && ~isFineResolution
                ...
                for k = 1:unitNum
                    unitRF = RFmap{k};
                    unitRF.OFF.OnSet = reshape(sum(reshape(unitRF.OFF.OnSet, ...
            ...
            for k = 1:unitNum
                unitsSpikeCounts(k, :, :, :) = RFmap{k}.OFF.OnSet;
            end
            ...
            if isBackgroundMoving
                if isFineResolution
                    jsonFileName = ['egocentric_unitsSpikeCounts_', sessionID, '_off', fileExtension];
                else
                    jsonFileName = ['egocentric_30_unitsSpikeCounts_', sessionID, '_off', fileExtension];
                end
            elseif isRotation
                if isFineResolution
                    jsonFileName = ['rotation_unitsSpikeCounts_', sessionID, '_off', fileExtension];
                else
                    jsonFileName = ['rotation_30_unitsSpikeCounts_', sessionID, '_off', fileExtension];
                end
            elseif isAllocentricPixelBins
                if isFineResolution
                    jsonFileName = ['allocentric_pixelbins_unitsSpikeCounts_', sessionID, '_off', fileExtension];
                else
                    jsonFileName = ['allocentric_pixelbins_30_unitsSpikeCounts_', sessionID, '_off', fileExtension];
                end
            else
                jsonFileName = ['regular_unitsSpikeCounts_', sessionID, '_off', fileExtension];
            end
            ...
            if ~isBackgroundMoving && ~isRotation && ~isAllocentricPixelBins && ~isUseRealCoordinate
                totalResponse = 0;
                for k = 1:unitNum
                    totalResponse = totalResponse + sum(RFmap{k}.OFF.OnSet(:));
                end
                if totalResponse == 0
        ...
        %%%%%egocentric%%%%%
            ...
            parfor k = 1:unitNum
                ...
                u = unitPool(k);
                rf = flipud(sum(RFmap{k}.OFF.OnSet, 3));
        ...
        %%%%%egocentric%%%%%
                ...
                unitPdf = fullfile(savePdfDir, sprintf('%03d_unit_%d_off.pdf', k, u));
...
function SaveRfPatternCsv(RFmap, unitPool, save_dir, total_deg, date, sessionID, isBackgroundMoving, isRotation, isAllocentricPixelBins, isFineResolution)
    ...
    if isBackgroundMoving
        if isFineResolution
            csvFile = fullfile(save_dir, ['egocentric_', sessionID, '_off.csv']);
        else
            csvFile = fullfile(save_dir, ['egocentric_30_', sessionID, '_off.csv']);
        end
    elseif isRotation
        if isFineResolution
            csvFile = fullfile(save_dir, ['rotation_', sessionID, '_off.csv']);
        else
            csvFile = fullfile(save_dir, ['rotation_30_', sessionID, '_off.csv']);
        end
    elseif isAllocentricPixelBins
        if isFineResolution
            csvFile = fullfile(save_dir, ['allocentric_pixelbins_', sessionID, '_off.csv']);
        else
            csvFile = fullfile(save_dir, ['allocentric_pixelbins_30_', sessionID, '_off.csv']);
        end
    else
        csvFile = fullfile(save_dir, ['regular_', sessionID, '_off.csv']);
    end
    ...
    sampleRf = flipud(sum(RFmap{1}.OFF.OnSet, 3));
