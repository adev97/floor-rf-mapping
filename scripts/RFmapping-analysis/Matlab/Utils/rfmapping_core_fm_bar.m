function rfmapping_core_fm_bar(params)
%RFMAPPING_CORE_FM_BAR Free-moving head-centric vertical-bar RF analysis.
%
...
% legacy JSON representation.
    ...
    motiveRotationDeg(~isfinite(motiveRotationDeg)) = 0;
    ...
    assert(isequal(azimuthEdgesDeg, -180:1:180), ...
        'FM bar azimuth edges must be -180:1:180.');
    assert(isequal(elevationEdgesDeg, -90:1:90), ...
        'FM bar elevation edges must be -90:1:90.');
...
function validateBarTrials(trials)
    requiredFields = {'Square_Size', 'Square_PositionX', ...
        'Square_PositionY', 'Square_Luminance'};
    for fieldIndex = 1:numel(requiredFields)
        assert(isfield(trials, requiredFields{fieldIndex}), ...
            'Stimulus trials are missing %s.', requiredFields{fieldIndex});
    end
    barWidthDeg = double([trials.Square_Size]);
    barCenterXDeg = double([trials.Square_PositionX]);
    barCenterYDeg = double([trials.Square_PositionY]);
    assert(all(ismember(barWidthDeg, [3 6 9 12])), ...
        'Square_Size must contain only the bar widths 3, 6, 9, or 12 degrees.');
    assert(all(ismember(barCenterXDeg, -174:12:174)), ...
        'Square_PositionX must use the 30 centers -174:12:174 degrees.');
    assert(all(barCenterYDeg == 0), ...
        'Square_PositionY must be 0 for the full-height bar stimulus.');
end
...
function geometry = validateCalibration(calibration)
    ...
    assert(strcmp(calibration.world_up_axis, 'Z'), ...
        'rfmapping_core_fm_bar requires a Z-up calibration.');
...
function [frame, rotationDeg, positionMm] = readMotiveRigidBody(csvPath, rigidBodyName)
    ...
    nameHeader = strsplit(header{4}, ',', 'CollapseDelimiters', false);
    measurementHeader = strsplit(header{7}, ',', 'CollapseDelimiters', false);
    axisHeader = strsplit(header{8}, ',', 'CollapseDelimiters', false);
    assert(numel(nameHeader) == numel(measurementHeader) && ...
        numel(nameHeader) == numel(axisHeader), ...
        'Motive CSV data headers must have equal column counts.');
    rotationColumn = motiveXyzColumns(nameHeader, measurementHeader, ...
        axisHeader, rigidBodyName, 'Rotation');
    positionColumn = motiveXyzColumns(nameHeader, measurementHeader, ...
        axisHeader, rigidBodyName, 'Position');

    numeric = readmatrix(csvPath, 'NumHeaderLines', 8);
    assert(size(numeric, 2) == numel(nameHeader), ...
        'Motive CSV numeric and header column counts must match.');
    frame = numeric(:, 1);
    rotationDeg = numeric(:, rotationColumn);
    positionMm = numeric(:, positionColumn);
...
function columns = motiveXyzColumns(nameHeader, measurementHeader, ...
        axisHeader, rigidBodyName, measurementName)
    columns = zeros(1, 3);
    axes = {'X', 'Y', 'Z'};
    for axisIndex = 1:3
        match = find(strcmp(nameHeader, char(rigidBodyName)) & ...
            strcmp(measurementHeader, measurementName) & ...
            strcmp(axisHeader, axes{axisIndex}));
        assert(isscalar(match), ...
            'Motive CSV must contain one %s %s column for rigid body %s.', ...
            measurementName, axes{axisIndex}, char(rigidBodyName));
        columns(axisIndex) = match;
    end
end
...
function [exposure, qa] = buildTrialExposure(trials, trialBoundarySec, ...
    ...
    for trialIndex = 1:trialCount
        ...
        [sourceWorldMm, sourcePixelCount] = barStimulusWorldPoints( ...
...
function [worldPointsMm, pixelCount] = barStimulusWorldPoints( ...
        trial, geometry, sourceScreenSizePix)
    screenWidthPix = sourceScreenSizePix(1);
    screenHeightPix = sourceScreenSizePix(2);
    degreePerPixel = 360 / screenWidthPix;
    barWidthPix = ceil(double(trial.Square_Size) / degreePerPixel);
    barHalfWidthPix = ceil(barWidthPix / 2);

    lowerX = screenWidthPix / 2 + ...
        ceil(double(trial.Square_PositionX) / degreePerPixel) - barHalfWidthPix;
    upperX = lowerX + barWidthPix;
    xPixel = max(1, lowerX):min(screenWidthPix, upperX);
    yPixel = 1:screenHeightPix;
    [xGrid, yGrid] = meshgrid(xPixel, yPixel);
    xGrid = xGrid(:);
    yGrid = yGrid(:);
    pixelCount = numel(xGrid);
    assert(pixelCount > 0, 'Stimulus bar is completely outside the source texture.');
...
function writeRfmap(params, calibrationText, motiveFrame, cameraFrameTimeSec, ...
    ...
    h5writeatt(params.rfmapOutputPath, '/', 'format', ...
        'rfmapping_fm_bar_hdf5_v1');
    ...
    h5writeatt(params.rfmapOutputPath, '/', 'spatial_grid', ...
        'full_sphere_1deg');
    h5writeatt(params.rfmapOutputPath, '/', 'stimulus_geometry', ...
        'vertical_bar_full_source_height');
    h5writeatt(params.rfmapOutputPath, '/', 'bar_width_handling', ...
        'pooled; each trial uses its recorded Square_Size');
    h5writeatt(params.rfmapOutputPath, '/', 'bar_widths_present_deg', ...
        unique(double([trials.Square_Size])));
