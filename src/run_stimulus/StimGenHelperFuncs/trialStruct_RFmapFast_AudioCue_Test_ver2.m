...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [trials] = trialStruct_RFmapFast_AudioCue_Test_ver2( stimType, table)
...
for i = 1:size(table,1)
    ...
    elseif strcmp(fieldname{i},'AV_V_A_ratio')
        constants.(fieldname{i}) = horzcat(table{i,2:4});
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
%         CuePosNum = (1+2*constants.Cue_Spatial_Jitter)^2;
%         % new_array_num = size(trial_arrays,1)+ 2*CuePosNum*(constants.TargetDistractor_OverPresentation-1);
%         % new_trial_arrays = zeros(new_array_num, size(trial_arrays,2)+1);
% 
%         targetXid = find(params.Square_PositionX==constants.Audio_TargetX);
%         target_params_array{1} = params.Square_PositionX(...
%             (targetXid-constants.Cue_Spatial_Jitter):(targetXid+constants.Cue_Spatial_Jitter));
%         targetYid = find(params.Square_PositionY==constants.Audio_TargetY);
%         target_params_array{2} = params.Square_PositionY(...
%             (targetYid-constants.Cue_Spatial_Jitter):(targetYid+constants.Cue_Spatial_Jitter));
%         
%         distractorXid = find(params.Square_PositionX==constants.Audio_DistractorX);
%         distractor_params_array{1} = params.Square_PositionX(...
%             (distractorXid-constants.Cue_Spatial_Jitter):(distractorXid+constants.Cue_Spatial_Jitter));
%         distractorYid = find(params.Square_PositionY==constants.Audio_DistractorY);
%         distractor_params_array{2} = params.Square_PositionY(...
%             (distractorYid-constants.Cue_Spatial_Jitter):(distractorYid+constants.Cue_Spatial_Jitter));
%         
%         p=1:2; ip=2:-1:1;
%         [target_array{ip}] = ndgrid(target_params_array{ip});
%         target_array = reshape(cat(length(p)+1,target_array{:}),[],length(p));
%         target_array = repmat(target_array, [constants.TargetDistractor_OverPresentation-1, 1]);
% 
%         [distractor_array{ip}] = ndgrid(distractor_params_array{ip});
%         distractor_array = reshape(cat(length(p)+1,distractor_array{:}),[],length(p));
%         distractor_array = repmat(distractor_array, [constants.TargetDistractor_OverPresentation-1, 1]);
% 
%         new_trial_arrays = [trial_arrays; target_array; distractor_array] ;
%         new_array_num = size(new_trial_arrays,1);
%         new_trial_arrays = [new_trial_arrays, zeros(new_array_num,1)];
%         
%         for ii=1:new_array_num
%             
%             if any(new_trial_arrays(ii,1)==target_params_array{1}) &&any(new_trial_arrays(ii,2)==target_params_array{2})
%                 new_trial_arrays(ii,3)=1;
%             end
%         end
%     
%         
%     trial_arrays = new_trial_arrays; 
% 
% 
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
audioIdx = trial_arrays(:,1)==constants.Audio_TargetX & trial_arrays(:,2)==constants.Audio_TargetY;
...
%         end
        ...
        if constants.Randomize == 1 && constants.Interleave ~=1
            ...
            if strcmp(stimType,'Receptive Field Mapping Audio Cue')
                
                new_trial_arrays = trial_arrays;
                
                AVnum = constants.AV_V_A_ratio(1);
                Vnum = constants.AV_V_A_ratio(2);
                Anum = constants.AV_V_A_ratio(3);
                
                Von = mod(i+round(AVnum/2),AVnum)< Vnum;
                Aon = mod(i,AVnum)< Anum;
                
                if Von
                    new_trial_arrays = [new_trial_arrays; [constants.Audio_TargetX, constants.Audio_TargetY, 0] ];
                end
                
                if Aon
                    new_trial_arrays = [new_trial_arrays; [100,100, 1] ];

                end
                
                trial_num = size(new_trial_arrays,1);
                ...
                for k=1:trial_num
                    ...
                    while 1
                        ...
                        current_X = new_trial_arrays(curIdx,1);
                        current_Y = new_trial_arrays(curIdx,2);
                        current_Cue = new_trial_arrays(curIdx,3);
                        
                        if previous_X{1} ~= current_X && previous_X{2} ~= current_X ...
                                && previous_Y{1} ~= current_Y && previous_Y{2} ~= current_Y
                            break;
                        end
                        count_n = count_n + 1;
                        if count_n > 10
                    ...
                    previous_X{2} = previous_X{1};
                    previous_X{1} = current_X;
                    previous_Y{2} = previous_Y{1};
                    previous_Y{1} = current_Y;
                    previous_Cue = current_Cue;
                end
                
            else
                randIndex = randperm(size(new_trial_arrays,1));
            end
            
            
            new_trial_arrays = new_trial_arrays(randIndex,:);
        end
        
        trial_arrays_cat = vertcat(trial_arrays_cat, new_trial_arrays);
...
%     end
    ...
    % back shift Audio Cue
    
    bb = constants.Audio_Frame_Backward;
    new_audio_cue = [trial_arrays((bb+1):end,3); zeros(bb,1) ];
    trial_arrays(:,3) = new_audio_cue;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     %end
    ...
    params_fields = {'Square_PositionX','Square_PositionY','Audio_Cue'};
