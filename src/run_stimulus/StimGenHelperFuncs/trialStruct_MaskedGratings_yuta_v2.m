...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [trials] = trialStruct_MaskedGratings_yuta_v2( stimType, table)
...
for i = 1:size(table,1)
    ...
    elseif strcmp(fieldname{i},'Mask_Center_XY')
        % params.(fieldname{i}) = linspace(table{i,2}, table{i,4}, table{i,3})';
        params.(fieldname{i}) = table{i,2};
    ...
    elseif strcmp(fieldname{i},'Contrast') && table{i,2} == table{i,4}
        ...
        constants.(fieldname{i}) = min(table{i,2}^table{i,4},1);
    ...
    elseif strcmp(fieldname{i},'Contrast') && table{i,3} ~= table{i,4}
        ...
        for exponent = table{i,3}:table{i,4}
            ...
            contrast(exponent) = table{i,2}^exponent;
        end
...
if ~isempty(fieldnames(params))
    ...
    % RANDOMIZE AND NO INTERLEAVE and no REPEAT
    if constants.Randomize == 1 && constants.Interleave ~=1 && constants.Repeats==0
        randIndex = randperm(size(trial_arrays,1));
        trial_arrays = trial_arrays(randIndex,:);
    end
    
    % REPEAT and RANDOMIZE AND NO INTERLEAVE
    if constants.Randomize == 1 && constants.Interleave ~=1 && constants.Repeats > 0
        ...
        for i=1:(constants.Repeats+1)
            ...
            for k=1:trial_num
                ...
                while 1
                    ...
                    current_positionIdx = trial_arrays(curIdx,2); % second column as the XYposition
