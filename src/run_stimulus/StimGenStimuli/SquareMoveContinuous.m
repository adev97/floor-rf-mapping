...
try
    ...
    for trial=1:numel(trials)
        ...
        % trial
            ...
            if trials(trial).OmitFlag==1
                omit_period_cell{trial} = trials(trial).Omit_Period;
            else
                omit_period_cell{trial} = [];
            end
...
%         for textureIdx=1:numel(SquarePosXPool)
    ...
    %         degPositionY = SquarePosYPool(textureIdx);
            degPositionX = trials(trial).Square_PositionX_vector(1);
            degPositionY = trials(trial).Square_PositionY_vector(1);
            
            
    X_shiftperframe = round(1/degPerPix * trials(trial).Square_VelocityX/60); 
    Y_shiftperframe = round(1/degPerPix * trials(trial).Square_VelocityY/60); 
...
%         end
    ...
    for trial=1:numel(trials)
        ...
        omit_period = omit_period_cell{trial};
...
%%%%%%%%%%%%%%%%%%%%%%%%%% DRAW GRATING TEXTURE %%%%%%%%%%%%%%%%%%%%%%%%%%%       
        ...
        % rect and calculate grating shifts etc
            ...
            runtime = vbl + duration(trial);
            
            if ~isempty(omit_period)
                omitstart = vbl + omit_period(1); 
                omitend = vbl + omit_period(2);  
            else
                omitstart = runtime + 100;
                omitend = runtime + 100;
            end
