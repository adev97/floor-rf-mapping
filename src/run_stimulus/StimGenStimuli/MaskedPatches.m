% function MaskedPatches(trials)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This func generates and draws a gaussian masked grating with parameters
...
% INPUTS:  TRIALSSTRUCT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Written by MSC 4-28-12 (Modified from DriftDemo2 in PTB)
% Modified by: MSC/
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [trials] = MaskedPatches(stimType,table)
 if nargin<1
    table = {'Spatial Frequency (cpd)', .04, .04, .04;...
              'Temporal Frequency (cps)', 3, 1, 3;...
              'Diameter (deg)', 20, 5, 20;...
              ...
              'Interleave', 0, [], [];...
              'Repeats', 0, [], []};
          stimType = 'Masked Grating';
    
 end
trials = trialStruct(stimType, table);
%Set the initialization screen preceeding the stimulus to be 1 sec in dur.
[trials(:).Initialization_Screen] = deal(1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
hAO=daq.createSession('ni');
hAO.Rate = 5000;
addAnalogOutputChannel(hAO,'Dev1',0:1,'Voltage');
outputSingleScan(hAO,[0 0]);
...
%%%%%%%%%%%%%%%%%%%%%% GET SPECIFIC MONITOR INFORMATION %%%%%%%%%%%%%%%%%%%
    ...
    grayPix = GammaCorrect(grayLum);%grayLum;%
...
%     % Gamma correct
%     load('GammaTable_Dell_G2410_c50_b50');
%     Screen('LoadNormalizedGammaTable',w,gammaTable*[1 1 1]);
...
%%%%%%%%%%%%%%%%%%%% GET STIMULUS TIMING INFORMATION %%%%%%%%%%%%%%%%%%%%%%
        ...
        optoDelay = trials(trial).Opto_Timing(1);
        optoWait = trials(trial).Opto_Timing(2);
        if isfield(trials, 'Led')
            bOptoTrial = trials(trial).Led;
        else
            bOptoTrial = false;
        end
        bOptoFlag = 1;
        if isfield(trials, 'Phase_Offset')
            fPhaseOffset = trials(trial).Phase_Offset;
        else
            fPhaseOffset = 0;
        end
...
%%%%%%%%%%%%%%%%%%%%% CONSTRUCT GRATING TEXTURE %%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if ~strcmp(trials(trial).Stimulus_Type, 'Blank')                    
            ...
            % make the grating texture and save to gratingtex cell array
            gratingtexInv{trial}=Screen('MakeTexture', w, grating);

        end
        
%%%%%%%%%%%%%%%%%%%%%%%% CONSTRUCT MASK TEXTURE %%%%%%%%%%%%%%%%%%%%%%%%%%%
%Now we will construct a mask texture. We need the mask type and the
% diameter of the mask
    ...
    if ~strcmp(trials(trial).Stimulus_Type, 'Blank')
        ...
        % Get mask type from the first trial in the trial struct array
        maskType = trials(1).Mask_Type{1};
            
        switch maskType % we will switch based on the type of mask in trial
                        % structure
            
            case 'Square' % User selected a square mask
                masktex{trial}=[];
                % Do Nothing because the dstRect where grating is drawn is
                % already a square by definition
                
            case 'Circular' % User selected a circular mask
                ...
                % each dimension so the mask will be centered about the x,y
                % grating position specified by the user)
                mask = ones(visibleSize+1, visibleSize+1, 2) * grayPix; 
            
                % make a 2-D grid over mask positions (mx,my) to calculate
                % the mask value
                [mx,my]=meshgrid(-visibleSize/2:visibleSize/2,...
                ...
                %make the mask texture
                masktexInv{trial}=Screen('MakeTexture', w, mask);

            
            case 'Gaussian' % User selected a Gaussian mask
                ...
                % Calculate mask transparency vals (i.e. the 3rd Dimension
                % over
                %the gridpoints. A choice is made here as to how to define
                %the width of the Gaussian the FWHM is
                %diamPix/(2*sqrt(2*log(2))) but this is much larger than
                %the user selected diameter because the this still leaves
                %too much area under gaussian curve. I am using the full
                %width at 1/100th the maximum gaussian height as the
                %definition for the gaussian width. This gives a nice
                %smooth gaussian drop of and gives the user a diameter much
                %closer to the width they asked for
            
                sigma = diamPix/(2*sqrt(2*log(100)));
                mask(:, :, 2)=255 * (1 - exp(-(((mx).^2/(2*sigma^2)))-...
                                                (((my).^2/(2*sigma^2)))));
                % Make the mask texture
                masktex{trial}=Screen('MakeTexture', w, mask);
        end
 ...
 %%%%%%%%%%%%%%% OBTAIN GRATING PARAMS FROM TRIALSSTRUCT %%%%%%%%%%%%%%%%%%
    ...
    if ~strcmp(trials(trial).Stimulus_Type, 'Blank')
        ...
        x = (trials(1).Mask_Center(1))*(1/degPerPix)+...
                    monitorInfo.screenSizePixX/2;
        y = -1*trials(1).Mask_Center(2)*(1/degPerPix)+...
...
%%%%%%%%%%%%%%%%%%%% DRAW DELAY GRAY SCREEN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    ...
    % delay
        ...
        if bFlag
            tLoopDelay = tLoopDelay - (delayPrev + durationPrev + waitPrev) + tLoopDelayPrev;
        else
            bFlag = 1;
        end
        
        delayTime = vbl + delay - tLoopDelay;
        optoDelayTime = delayTime - optoDelay;
        tLoopDelayPrev = tLoopDelay;
        delayPrev = delay;
    ...
    % to take possible timing jitter or roundoff-errors into account.
        while (vbl < delayTime)
            if bOptoTrial && bOptoFlag
                if vbl >= optoDelayTime
                    outputSingleScan(hAO,[0 10]);
                    bOptoFlag = 0;
                end
            end
            % Draw a gray screen
...
%%%%%%%%%%%%%%%%%%%%%%%%%% DRAW GRATING TEXTURE %%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if ~strcmp(trials(trial).Stimulus_Type, 'Blank')
            ...
            % Set analog output to code for stimulus ID
            outputSingleScan(hAO,[trials(trial).StimID*10 bOptoTrial*10]); %extend to +-10V if necessary
            
            for orientation = orientations
                
                runtime = runtime + duration/numel(orientations);
            
                while (vbl < runtime)
                    % calculate the offset of the grating and use the mod func
                    % to ensure the grating snaps back once the border is
                    % reached
                    xoffset = mod(n*shiftperframe+fPhaseOffset*pxPerCycle,pxPerCycle);
                    n = n+1;

                    switch trials(trial).Inversion
                        case 0 % no inversion
                            dstRect = [0 0 diamPix+1 diamPix+1];
                            mDstRect=CenterRectOnPoint(dstRect,x,y);
                        ...
                        case 1 % inversion
                            ...
                            srcRect = [xoffset 0 xoffset + visibleSize+1 visibleSize+1];
                            mSrcRect = [0 0 visibleSize+1 visibleSize+1];

                            % Draw the grating texture for this trial to the dest
                            % rectangle
                            Screen('DrawTextures', w, gratingtexInv{trial}, srcRect,...
                                dstRect, orientation);

                            % Draw the mask texture over the grating texture
                            if ~isempty(masktex{trial})
                                Screen('DrawTextures', w, masktexInv{trial}, mSrcRect,...
                                    mDstRect, orientation);
                            end
                    end
                    ...
                    % explanation
                    FlipCheck(w, screenRect, [whitePix, blackPix], n, trial)
            ...
            outputSingleScan(hAO,[0 bOptoTrial*10]); %extend to +-10V if necessary
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % JAVA OUT OF MEMORY ERRORS!!!
        if ~strcmp(trials(trial).Stimulus_Type, 'Blank')
            ...
            Screen('Close', gratingtexInv{trial})
                if ~isempty(masktex{trial})
                    Screen('Close', masktex{trial})
%                     Screen('Close', masktexInv{trial})
                end
        end
    end
    %
    ...
    % If the user has prematurely exited the trials loop we still must
    % close all the textures to prevent memory errors. If the loop
    % iteration completed then the texture was closed. If the loop did not
    % complete then it is still in memory
    
    if exist('gratingtex','var')
        disp('Textures cleared from memory')
        clear('gratingtex')
        clear('masktex')
    end
